...
// How recent counts as "current". Anything older is hidden in the default views.
const NEWS_WINDOW_DAYS = 60;
// The Competitor Deep-Dive looks back ~2 years.
const DEEP_WINDOW_DAYS = 730;

let tracker = { generated_at: null, items: [] };
let currentView = "executive";
let deepDiveCompany = "";   // selected competitor in the deep-dive view
...
const titleMap = {
  ...
  deepdive: ["Competitor Deep-Dive", "Pick a competitor: 2 years of news, ranked by relevance."],
...
function normalizeItems() {
  tracker.items = (tracker.items || []).map(item => {
    ...
    return {
      ...
      relevance: (item.relevance === null || item.relevance === undefined) ? 0 : Number(item.relevance),
...
function setupFilters() {
  ...
  // Deep-dive uses the full 2-year set of companies, not just the 60-day ones.
  const allCompanies = [...new Set(tracker.items.map(i => i.company))].sort();
  const ddSelect = document.getElementById("deepDiveCompany");
  if (ddSelect) {
    ddSelect.innerHTML = '<option value="">Choose a competitor…</option>' +
      allCompanies.map(c => `<option>${escapeHtml(c)}</option>`).join("");
    ddSelect.addEventListener("change", e => { deepDiveCompany = e.target.value; renderDeepDive(); });
  }
}
...
/* ---------- Competitor Deep-Dive: 2-year window, relevance-ranked ---------- */
function renderDeepDive() {
  const el = document.getElementById("deepDiveList");
  if (!el) return;
  const meta = document.getElementById("deepDiveMeta");

  if (!deepDiveCompany) {
    if (meta) meta.textContent = "";
    el.innerHTML = emptyHtml("Choose a competitor above to see up to 2 years of their news, ranked by relevance.");
    return;
  }

  const q = document.getElementById("searchInput").value.toLowerCase().trim();
  const items = tracker.items
    .filter(i => i.company === deepDiveCompany && i.ageDays <= DEEP_WINDOW_DAYS)
    .filter(i => !q || textOf(i).includes(q))
    // relevance first, date as tiebreak (newest wins ties)
    .sort((a, b) => {
  ...
  if (meta) {
    const threats = items.filter(i => i.bucket === "Threat").length;
    meta.textContent = `${items.length} items over ~2 years · ${threats} scored threats · ranked by relevance`;
  }
  renderList("deepDiveList", items, { showRelevance: true });
}
...
function renderList(id, items, opts = {}) {
  ...
  items.forEach(item => {
    ...
    node.querySelector(".tags").innerHTML = (item.tags || [])
      ...
      + (opts.showRelevance ? `<span class="tag rel">relevance ${item.relevance.toFixed(1)}</span>` : "");
